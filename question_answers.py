...
class QuestionMaker:
    ...
    # Funtion to render the question container
    def q_render(self):
        wt.display_board.surface.blit(self.question_container, self.question_container_placment)
    ...
    def __init__(self, question_num, question, font_style = JURA_BOLD, font_size = QUESTION_SIZE, q_contain_colr = (247, 255, 247), q_num_col = QUESTION_NUM_COLOUR, q_col = QUESTION_COLOUR):
        ...
        self.question_container = pg.Surface((self.width, self.height))
        self.question_container_placment = self.question_container.get_rect(center = (self.question_container_x, self.question_container_y))
        self.question_container.fill(self.ques_container_col)
        ...
        self.question_container.blit(self.question_num, self.question_num_placment)
        ...
        self.question_container.blit(self.question, self.question_placment)


# Creating the questions
Q1 = QuestionMaker("Q1", "What is the formula")
Q2 = QuestionMaker("Q2", "What is the answer")
...
class AnswerMaker:
    ...
    def __init__(self, answers, correct_answer, opt_colrs = opt_colr_list, opt_font = JURA_BOLD, opt_size = opt_font_size, ans_font = JURA_MEDIUM, font_colr = opt_font_colr):
        # Choosing a random colour for the options
        random.shuffle(opt_colrs)
        ...
        self.ans_y = int(self.height / 2)
            # Option A container
        self.opt_A_state = False
        self.opt_A_cont = pg.Surface((self.width, self.height))
        self.opt_A_cont.fill(opt_colrs[0])
        self.opt_A_cont_plac = self.opt_A_cont.get_rect(topleft = (527, 394))
        self.opt_A = self.font_sty_opt.render("A", self.anitalias, font_colr)
        self.opt_A_plac = self.opt_A.get_rect(center = (self.opt_x, self.opt_y))
        self.opt_A_cont.blit(self.opt_A, self.opt_A_plac)
        self.ans_A = self.font_sty_ans.render(answers[0], self.anitalias, font_colr)
        self.ans_A_plac = self.ans_A.get_rect(center = (self.ans_x, self.ans_y))
        self.opt_A_cont.blit(self.ans_A, self.ans_A_plac)
            # Option B container
        self.opt_B_state = False
